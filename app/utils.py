...
def hash_pwd(pwd):
    ...
    prehashed_pwd = hashlib.sha256(pwd_bytes).digest()
    hashed_pwd = bcrypt.hashpw(prehashed_pwd, salt=bcrypt.gensalt(12))
    return hashed_pwd.decode('utf-8')
   

def verify_pwd(pwd, hashed_pwd):
    prehashed_pwd = hashlib.sha256(pwd.encode('utf-8')).digest()
    hashed_pwd_bytes = hashed_pwd.encode("utf-8")
    return bcrypt.checkpw(prehashed_pwd, hashed_pwd_bytes)
